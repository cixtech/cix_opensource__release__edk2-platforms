...
static bool check_opp_table(void)
{
    ...
    for (i = 0; i < ARRAY_LENGTH(dom_opps); i++) {
        ...
        for (j = 0; j < config->size; j++) {
            if (config->opp_table[j].level == 0 ||
                    config->opp_table[j].voltage == 0 ||
                    config->opp_table[j].frequency == 0) {
                printf("domain %zu: OPP entry %u: level, voltage or frequency must not be zero\n",
                    i, j);
                return false;
            }
        }
    ...
    if (!config) {
        printf("DSU domain OPP mising. Skip OPP checking\n");
        return true;
    }
...
static void dump_fan_config(pm_config_fan_t* config, uint32_t num)
{
    for (uint32_t i = 0; i < num; i++) {
        ...
        if (config[i].fan_valid.fields.valid == PM_CONFIG_INVALID) {
            ...
            printf("\tsensor_id:\t");dump_valid_flag(config[i].sensor_id);
        }
    }
}
...
static void dump_log_config(pm_config_log_uart_t * log_config)
{
...
static void dump_op_mode_config(config_data_t *op_mode)
{
    if (op_mode->fields.valid == PM_CONFIG_INVALID) {
        printf("INVALID\n");
        return;
    }
    printf("\n\top_mode: %u\n", op_mode->fields.raw_data);
}

static void dump_config()
{
    pm_export_config_t *config = &g_config.config;
    uint32_t crc1 = g_config.crc1;
    uint32_t crc2 = g_config.crc2;

    if (g_config.length > sizeof(g_config)) {
        printf("config size %u invalid\n", g_config.length);
        return;
    }
    ...
    bool crc_valid;
    uint64_t ck = (((uint64_t)crc1 << 32) | (uint64_t)crc2);

    g_config.crc1 = 0;
    g_config.crc2 = 0;
    crc_valid = double_check_sum(&g_config, g_config.length, &ck, true);
    if (!crc_valid) {
        printf("CRC invalid - expect 0x%08x%08x, actual 0x%08x%08x\n",
            (uint32_t)(ck >> 32), (uint32_t)(ck & 0xFFFFFFFF),
            crc1, crc2);
    } else {
        printf("CRC valid\n");
    }
    ...
    printf("Length : %u, sizeof(g_config) : %zu, sizeof(opps) : %zu\n", g_config.length, sizeof(g_config), sizeof(config->opp_config.opps));
    printf("pmic config:\n");
    ...
    printf("OP mode config:");
    dump_op_mode_config(&config->op_mode);
...
int main(int argc, char **argv)
{
    ...
    if (argc > 1) {
        ...
        FILE *bin_file;
        if ((bin_file = fopen(argv[1], "rb")) == NULL) {
            printf("\n\nfile %s open failed!\n", argv[1]);
            return -1;
        }
    ...
    // pmic config
#if PM_PMIC_CONFIG
    if (!validate(&vad_pmic_opp_max, &pmic_config.opp_max[0])) {
        printf("Bad PMIC opp max\n");
        return -1;
    }
    for (size_t i = 0; i < OPP_PWR_RAIL_MAX; i++) {
        if (!validate_dpm_pwr_rail_cfg(&pmic_config.edp_cfg[i])) {
            printf("Bad power rail config\n");
            return -1;
        }
    }
    ...
    if (!validate_pvt_config(&pvt_config)) {
    ...
    // fan config
    if (!validate_fan_table(fan_config, MAX_FAN_NUM)) {
...
#if PM_GPIO_CONFIG
    if (!validate_gpio_config(&gpio_config)) {
    ...
    if (all_dom_valid) {
        ...
        config->opp_config.opp_valid = (PM_CONFIG_VALID | OPP_PARTIAL_VALID);
    }
#endif
    ...
    memcpy(&config->noc_idle_config, &noc_idle_config_data, sizeof(config->noc_idle_config));
#endif
    ...
    if (!validate_op_mode(&op_mode_config)) {
        printf("Invalid OP mode\n");
        return -1;
    }
    memcpy(&config->op_mode, &op_mode_config, sizeof(config->op_mode));
#endif
    ...
    _Static_assert(sizeof(g_config) <= PM_CONFIG_BIN_SIZE, "g_config exceeds size limit\n");
